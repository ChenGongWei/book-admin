		...
		<view class="uni-container">
			<uni-forms ref="resetPwdForm" v-model="password" :rules="rules" @submit="submit">
				<uni-forms-item :label="旧密码" name="oldPassword" labelWidth="85">
					<input class="uni-input-border" type="password" :placeholder="请输入旧密码" v-model="password.oldPassword" />
				</uni-forms-item>

				<uni-forms-item :label="新密码" name="newPassword" labelWidth="85">
					<input class="uni-input-border" :password="showPassword" :placeholder="请输入新密码" v-model="password.newPassword" />
					<text class="uni-icon-password-eye pointer" :class="[!showPassword ? 'uni-eye-active' : '']" @click="changePassword">&#xe568;</text>
				</uni-forms-item>

				<uni-forms-item :label="确认密码" name="passwordConfirmation" labelWidth="85" :errorMessage="errorMessage">
					<input @confirm="submitForm" class="uni-input-border" :password="showPasswordAgain"
					 :placeholder="请再次输入新密码" v-model="password.passwordConfirmation" />
...
<style>
	...
	.login-button-width,.uni-button-full {
		width: 184px;
	}
</style>
